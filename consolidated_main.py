...
class ConsolidatedStockDiscoveryEngine:
    ...
    async def run_discovery_pipeline(self, timeframe: str = "daily") -> Dict[str, Any]:
        ...
        try:
            ...
            # Analyze ALL identified stocks, not just those with market data
    ...
    async def display_results(self, results: Dict[str, Any]):
        ...
        print(f"   (These are stocks identified by AI analysis - final recommendations below apply additional filters)")
        
        # Display top 10 opportunities from LLM analysis
        stock_opportunities = results.get('stock_analysis', {}).get('analysis', {})
        sorted_opportunities = sorted(
            stock_opportunities.items(), 
            key=lambda x: x[1].get('impact_magnitude', 0) * x[1].get('confidence', 0), 
            reverse=True
        )[:10]
        
        for i, (stock, stock_info) in enumerate(sorted_opportunities, 1):
