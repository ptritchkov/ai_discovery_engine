...
class SentimentAnalyzer:
    ...
    async def _combine_sentiment_sources(
        ...
        try:
            ...
            # Extract stocks mentioned in news for sentiment mapping
            news_stocks = await self._extract_stocks_from_news(news_sentiment)
            all_stocks.update(news_stocks.keys())
            ...
            for stock in all_stocks:
                ...
                news_data = news_stocks.get(stock, {})
                ...
                has_social = bool(social_data)
                has_market = bool(market_data)
                has_news = bool(news_data)
                
                if has_social and has_market and has_news:
                    social_weight, market_weight, news_weight = 0.3, 0.4, 0.3
                elif has_social and has_news:
                    social_weight, market_weight, news_weight = 0.5, 0.0, 0.5
                elif has_market and has_news:
                    social_weight, market_weight, news_weight = 0.0, 0.6, 0.4
                elif has_news:
                    social_weight, market_weight, news_weight = 0.0, 0.0, 1.0
                elif has_social:
                    social_weight, market_weight, news_weight = 1.0, 0.0, 0.0
                elif has_market:
                    social_weight, market_weight, news_weight = 0.0, 1.0, 0.0
                else:
                    social_weight, market_weight, news_weight = 0.0, 0.0, 0.0
                ...
                confidence = 0.0
                if social_data:
                    confidence += social_data.get('confidence', 0.0) * social_weight
                if market_data:
                    confidence += market_data.get('confidence', 0.0) * market_weight
                if news_data:
                    confidence += news_data.get('confidence', 0.7) * news_weight  # Default news confidence
    ...
    async def _extract_stocks_from_news(self, news_sentiment: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """Extract stock-specific sentiment from news articles."""
        try:
            stock_sentiments = {}
            article_sentiments = news_sentiment.get('article_sentiments', [])
            ...
            stock_patterns = {
                ...
                'NVDA': ['nvidia', 'gpu', 'ai chip', 'graphics', 'jensen huang'],
                'NFLX': ['netflix', 'streaming', 'content', 'subscriber'],
                'TSM': ['taiwan semiconductor', 'tsmc', 'chip', 'semiconductor'],
                'INTC': ['intel', 'processor', 'cpu', 'semiconductor'],
                'AMD': ['amd', 'ryzen', 'radeon', 'processor'],
                'CRM': ['salesforce', 'crm', 'cloud software'],
                'ORCL': ['oracle', 'database', 'cloud'],
                'ADBE': ['adobe', 'photoshop', 'creative'],
                'XLF': ['financial', 'bank', 'finance', 'jpmorgan', 'goldman'],
                'XLE': ['energy', 'oil', 'exxon', 'chevron', 'petroleum'],
                'XLI': ['industrial', 'manufacturing', 'boeing', 'caterpillar'],
                'XLY': ['consumer', 'retail', 'amazon', 'home depot'],
                'XLK': ['technology', 'tech', 'software', 'hardware']
            }
            ...
            for article in article_sentiments:
                ...
                for symbol, keywords in stock_patterns.items():
                    ...
                    if any(keyword in content for keyword in keywords):
                        ...
                        # Add this article's sentiment to the stock
                        stock_sentiments[symbol]['sentiment_score'] += article.get('combined_score', 0.0)
                        stock_sentiments[symbol]['article_count'] += 1
                        stock_sentiments[symbol]['articles'].append({
                            'title': article.get('title', ''),
                            'sentiment': article.get('combined_score', 0.0)
                        })
            ...
            for symbol, data in stock_sentiments.items():
                if data['article_count'] > 0:
                    ...
                    data['confidence'] = min(0.8, 0.3 + (data['article_count'] * 0.1))  # Higher confidence with more articles
            
            return stock_sentiments
            
        except Exception as e:
            self.logger.error(f"Error extracting stocks from news: {str(e)}")
            return {}
