...
class LLMAnalyzer:
    ...
    async def identify_affected_stocks(self, news_data: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            ...
            # Analyze more stocks for comprehensive coverage
            max_stocks = min(len(validated_stocks), 15)  # Increased from 5 to 15
            selected_stocks = validated_stocks[:max_stocks]
            self.logger.info(f"Identified {len(validated_stocks)} potentially affected stocks, analyzing top {len(selected_stocks)}")
...
"""
    ...
    def _fallback_stock_identification(self, news_data: List[Dict[str, Any]]) -> List[str]:
        ...
        return list(stocks)[:5]  # Limit to 5 for testing
